...
trait VerifiesSocialUsers
{
    ...
    /** * @param $facebookUser * @return mixed */
    ...
    /** * @param $socialUser * @return bool */
    private function verifyUserIds($socialUser)
    {
        return (SocialProvider::where('source_id', $socialUser->id)->where('user_id', Auth::id())->where('source', $this->provider)->exists()) ? true : false;
    }

    /** * @param $socialUser * @return mixed */
    private function userWhereEmailMatches($socialUser)
    {
        return User::where('email', $socialUser->email)->first();
    }

    /** * @param $socialUser * @param $authUser * @return mixed */
    private function matchesIds($socialUser, User $authUser)
    {
        return $authUser->socialProviders()->where('source', $this->provider)->where('source_id', $socialUser->id)->first();
    }
}
